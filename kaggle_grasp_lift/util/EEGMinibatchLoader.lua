...
function EEGMinibatchLoader.glob_raw_data_files(data_dir, num_test, num_val)
    -- XXX: num_test and num_val not used?
    local data = {}
    local val_data = {}
    local test_data = {}
    ...
    for file in lfs.dir(data_dir) do
        if file:find('data.csv.val') then -- validation file found
            table.insert(val_data, path.join(data_dir, file))
        elseif file:find('data.csv.test') then -- do nothing if test file found?
        elseif file:find('data.csv') then -- training file found
